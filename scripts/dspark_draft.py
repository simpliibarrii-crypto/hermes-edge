...
future tokens, which the main model then verifies in parallel.

IMPORTANT: LiteRT-LM has BUILT-IN Multi-Token Prediction (MTP) which is the
preferred approach for speculative decoding. Enable it by setting
enable_mtp=True on the model or using --enable_mtp with litert-lm CLI.

This module is retained for documentation and for environments where MTP
is not available. The draft model uses n-gram for ultra-light prediction.

Usage (MTP - recommended):
    model = LiteRTModel("model.litertlm")
    model.enable_mtp = True  # enables built-in speculative decoding (~2.2x)

Usage (fallback):
    from dspark_draft import DSparkDraftEngine, NGramDraftModel, DSparkConfig
    draft = NGramDraftModel(vocab_size=32000)
    engine = DSparkDraftEngine(model, draft)
"""
